...
// Layout rhythm for this page: space-y-8 between major sections (greeting,
// banner, quick-log, hero, raised module, flat grouped sections) — see the
// outer wrapper below. Flat grouped-list sections use no gap between their
// own rows (divide-y instead), consistent with the row pattern already
// hand-rolled on the Device page.
...
export default function HomePage() {
  ...
  const hasTrendOrBaseline = !!deviceId;
  const latestReadingCaption = liveReading
    ? `${parseServerTime(liveReading.time).toLocaleTimeString("th-TH", { hour: "2-digit", minute: "2-digit" })} · ${fmtAcetone(liveReading.acetone_delta_mv)} ${unitLbl}`
    : null;

  return (
    <div className="max-w-md mx-auto px-4 pt-5 pb-24 space-y-8">
      {/* Greeting */}
      ...
      <div className="space-y-3">
        ...
        {/* Quick log — replaces the old global "+" menu's "Log reading" with
            four specific, self-explanatory actions. */}
        <QuickLogRow />
      </div>

      {/* HERO — Metabolic Flexibility is the one dominant number on this page;
          breath acetone (today's max) is a supporting stat inside the same
          card rather than a second competing "lg" card. */}
      <Card tier="hero" padding="lg">
        ...
        {/* Supporting stat: today's breath acetone snapshot */}
        <div className="mt-5 pt-4 border-t border-border-soft flex items-center gap-4">
          <AcetoneRing value={heroValue} label={heroLabel} size={72} />
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-1.5">
              <p className="text-xs text-text-muted font-semibold uppercase tracking-widest">
                Breath acetone · วันนี้
              </p>
            ...
            <div className="flex items-center gap-2 mt-1">
              <div className={`h-2 w-2 rounded-full shrink-0 ${liveConnected ? "bg-mint-500 animate-pulse" : "bg-text-disabled"}`} />
              <span className="text-xs text-text-muted truncate">
                {liveConnected ? (
                  <>Live {liveReading && `· ${fmtAcetone(liveReading.acetone_delta_mv)} ${unitLbl}`}</>
                ) : heroCount > 0 ? (
            ...
            {/* Latest reading, folded in here instead of its own section —
                redundant with the live line above and with /breathing when
                nothing new happened today. */}
            {latestReadingCaption && (
              <p className="text-[11px] text-text-disabled mt-1">
                {t("health.todaySessions")}: {latestReadingCaption}
              </p>
            )}
          </div>
        </div>
      ...
      {/* Trend + baseline — one raised module, two compact glance rows.
          Full detail for each still lives on /trends. */}
      {hasTrendOrBaseline && (
        <Card tier="raised" padding="md" className="space-y-3">
          <TrendClassCard deviceId={deviceId} sessions={14} bare />
          <div className="border-t border-border-soft" />
          <BaselineCard deviceId={deviceId} bare />
        </Card>
      )}

      {/* Everything below is flat/grouped — secondary content, no card chrome
          of its own, differentiated by spacing and type instead. */}
      <div className="space-y-8">
        {/* Quests */}
        <div>
          <Card tier="flat" padding="none" className="flex items-center gap-3 py-1">
        ...
        {/* Today — metrics + categories, one flat grouped list of rows */}
        <div>
          <p className="text-xs text-text-muted font-semibold uppercase tracking-widest mb-1">Today</p>
          <div className="divide-y divide-border-soft">
            <TodayMetricCard kind="weight" variant="row" />
            <TodayMetricCard kind="steps" variant="row" />
            <TodayMetricCard kind="calories" variant="row" />
            <CategoryCard
              variant="row"
              icon="🌬"
              title="Breathing"
              ...
              iconBg="#00C896"
            />
            <CategoryCard variant="row" icon="📈" title="Trend" value="7-day" sub="ดูแนวโน้ม" href="/trends" iconBg="#3B82F6" />
            <CategoryCard variant="row" icon="💤" title="Sleep" value="—" iconBg="#A855F7" comingSoon />
            <CategoryCard variant="row" icon="❤️" title="Heart Rate" value="—" iconBg="#FF3B4A" comingSoon />
          </div>
        </div>
      </div>
    </div>
