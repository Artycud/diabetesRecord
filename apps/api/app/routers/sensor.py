...
from app.services.report_service import (
    SessionSummary, BaselineOut, ReportUser, SensorReport,
)
...
# SessionSummary + the summary-building logic now live in report_service.py
# (shared with the doctor-facing report route) — this stays a thin wrapper.
...
async def list_sessions(
    ...
    return await report_service.get_session_summaries(db, user, days)
...
# BaselineOut now lives in report_service.py (shared with the doctor-facing
# report route).
...
# ReportUser/SensorReport + the build logic now live in report_service.py
# (shared with the doctor-facing GET /doctor/patients/{id}/report route).
...
async def get_health_report(
    ...
    return await report_service.build_health_report(
        db, user, device_id, session_days, log_days, trend_readings,
    )
